...
function sendMessage() {
    ...
    if (message !== "") {
        addUserMessage(message);  // 调用函数添加用户消息
        userInput.value = "";
    }
}
        ...
        case "🎯 PCを選ぶ":
            ...
            case "✨ AIおすすめ":
                setTimeout(() => {
                    ...
                    addBotMessage("🆕 Lenovo Yoga Slim 7<br>🆕 Surface Pro 9");
...
function handleUserInput(message) {
    if (message.includes("おすすめ")) {
        const product = "Acer Nitro 5"; // 推荐的产品
    ...
    // 针对 CG 专门学生的提问
    else if (message.includes("CG専門の学生") || message.includes("パソコンを購入すれば良いですか")) {
        const recommendedProduct = "MacBook Pro 16.2 M4 Pro 24GB 512GB (2024)";
        addBotMessage("CG専門の学生としてパソコンを選ぶ際は、3Dモデリング、レンダリング、動画編集などの重い作業に対応できる高性能なスペックが必要です。<br>おすすめパソコンは<br>" + recommendedProduct + "です！");

        // 自动填入搜索栏并触发搜索
        autoFillSearchBar(recommendedProduct);

        // 添加刷新按钮
        updateButtons(["🔄 他のおすすめを見る", "🏠 ホームへ戻る"]);

    } else {
        addBotMessage("すみません、その内容には対応できません。メニューから選んでください。");
        updateButtons(["🎯 PCを選ぶ", "✨ AIおすすめ", "🔥 人気商品", "🚚 注文・配送"]);
    }
}
// 自动填入搜索栏并触发搜索
